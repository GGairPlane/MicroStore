...
def download_file(reply):
    try:
        ...
        with filedialog.asksaveasfile(mode="wb", initialfile=os.path.basename(reply[1].decode())) as f:
            f.write(reply[2])
...
def rename(old_name):
    ...
    root.mainloop()
    name = name.replace('~', '')
    return old_name.encode() + b'~' + os.path.join(os.path.split(old_name)[0], name).encode()
